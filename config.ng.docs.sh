...
module_options+=(
    ...
    ["see_function_table_md,ref_link"]="./README.md"
    ...
    ["see_function_table_md,status"]="Candidate"
        ["see_function_table_md,doc_link"]=""
)


function see_function_table_md() {
     mod_message="| Description | Example | Contributor | Reference | Documents | Status |\n"
    mod_message+="|:----------- | ------- | ----------- |:---------:|:---------:|:------:|\n"
    # Iterate over the options
    for key in "${!module_options[@]}"; do
        # Split the key into function_name and type
        ...
        if [[ "$type" == "feature" ]]; then
            ...
            ref_link_md=$([[ -n "$ref_link" ]] && echo "[Reference]($ref_link)" || echo "X")
            doc_link_md=$([[ -n "$doc_link" ]] && echo "[Document]($doc_link)" || echo "X")
            status_md=$([[ ( -z "$ref_link" && -z "$doc_link") ]] && echo "Incomplete" || ([[ ( -n "$ref_link" && -n "$doc_link") ]] && echo "Need_Review" || echo "$status" ) )          
            mod_message+="| ${module_options["$function_name,desc"]} | ${module_options["$function_name,example"]} | ${module_options["$function_name,author"]} | $ref_link_md | $doc_link_md | $status_md | \n"
        fi
    done
...
module_options+=(
    ...
    ["generate_readme,ref_link"]="https://github.com/armbian/configng/blob/main/lib/armbian-configng/documents.sh"
    ["generate_readme,feature"]="generate_readme"
    ["generate_readme,desc"]="Generate this README.md file."
    ["generate_readme,example"]="generate_readme"
    ["generate_readme,status"]="candidate"
...
EOF
#cat << EOF > "$script_dir/helper.list.json" 
#$(generate_json)
#EOF
