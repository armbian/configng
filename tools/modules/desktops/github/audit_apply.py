...
def run_claude(*, client, model, max_tokens, system_prompt, user_prompt,
    ...
    while True:
        ...
        # per-minute input-token budget. The budget is enforced by the
        # API as a sliding window of input tokens used in the last 60s.
        #
        ...
        # call's input size, plus a floor for the very first call.
        #
        # If the next call alone exceeds the per-minute budget, the
        # only safe pace is one call per minute regardless of the
        # window. Just sleep 60s and let the SDK's retry-after
        # handling cover any tail spike. Otherwise, sleep until the
        # oldest entry ages out far enough to fit the new one.
        now = time.monotonic()
