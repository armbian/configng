					...
					1) # ZRAM Settings submenu
						while true; do
							...
							local zram_choice=$(dialog_menu "ZRAM Settings" \
								...
								"4" "Max Devices - Current: ${max_devices} (recommended: ${cpu_cores})" \
					...
					5) # Save & Apply
						...
						if dialog_yesno "Save Configuration" "$config_preview"; then
							...
							# Write new configuration
							{
								...
								echo ""
								echo "# enable the armbian-zram-config service?"
								echo "ENABLED=${zram_enabled}"
								echo ""
								echo "# Zram swap enabled by default, unless set to disabled"
								echo "SWAP=${zram_swap_enabled}"
								echo ""
								echo "# percentage of zram used as swap compared to physically available DRAM"
								echo "ZRAM_PERCENTAGE=${zram_percentage}"
								echo ""
								echo "# percentage of DRAM available to zram"
								echo "MEM_LIMIT_PERCENTAGE=${mem_limit_percentage}"
								echo ""
								echo "# create how many zram devices max for swap"
								echo "ZRAM_MAX_DEVICES=${max_devices}"
								echo ""
								echo "# Which algorithm for zram based swapping"
								echo "SWAP_ALGORITHM=${swap_algorithm}"
								echo ""
								echo "# Which algorithm to choose for zram based ramlog partition"
								echo "RAMLOG_ALGORITHM=${ramlog_algorithm}"
								echo ""
								echo "# Which algorithm to choose for zram based /tmp"
								echo "TMP_ALGORITHM=${tmp_algorithm}"
								echo ""
								echo "# Size for zram based /tmp"
