...
module_options+=(
	...
	["module_desktop,example"]="install remove disable enable status manual auto help"
...
function module_desktop() {
	...
	case "$1" in
		"${commands[0]}")
			...
			# install desktop
			pkg_install -o Dpkg::Options::="--force-confold" --install-recommends armbian-${DISTROID}-desktop-${de}
