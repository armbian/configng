	...
	case "$1" in
		"${commands[0]}")
			...
			if [[ ${SWAG_URL} && $? -eq 0 ]]; then
				...
				if ! docker network ls | grep -q lsio; then
		...
		;;
		"${commands[4]}")
			SWAG_USER=$($DIALOG --title "Secure webserver with .htaccess username and password" --inputbox "\nHit enter for USERNAME defaults" 9 70 "armbian" 3>&1 1>&2 2>&3)
			SWAG_PASSWORD=$($DIALOG --title "Enter new password for ${SWAG_USER}" --inputbox "\nHit enter for auto generated password" 9 70 "$(tr -dc 'A-Za-z0-9=' < /dev/urandom | head -c 10)" 3>&1 1>&2 2>&3)
			docker exec -it swag htpasswd -c /config/nginx/.htpasswd ${SWAG_USER} ${SWAG_PASSWORD}
		;;
		"${commands[5]}")
