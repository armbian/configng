...
function module_zerotier() {
	...
	case "$1" in
		...
		"${commands[1]}")
			...
			echo "deb http://download.zerotier.com/debian/$(lsb_release -cs) $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/zerotier.list
			pkg_update
		...
		"${commands[7]}")
			systemctl is-active zerotier-one >/dev/null 2>&1 && echo -e "\033[0;32m****** Active *****\033[0m" || echo -e "\033[0;31m****** Inactive *****\033[0m"
		;;
